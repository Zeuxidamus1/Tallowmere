...
export function InventoryGrid({slots,level,mode="equip",onActivate,onItemContext,onMove}:{
  slots:InventorySlots; level:number; mode?:"equip"|"deposit"|"sell"; onActivate?:(id:ItemId,index:number)=>void;
  ...
  const activateItem = (id:ItemId,index:number,eventTime:number) => {
    if (eventTime-lastDragEndedAt.current<180) return;
    onActivate?.(id,index);
  };
  return <div className="inventory-grid" aria-label={`Inventory, ${used} of 28 slots used`}>
    {Array.from({length:28}).map((_,index) => {
      ...
      if (itemId && ITEMS[itemId].kind!=="resource") {
        ...
        const title = mode==="deposit" ? `Deposit ${GEAR[gearId].name}` : mode==="sell" ? `Select ${GEAR[gearId].name} to sell` : locked ? `${GEAR[gearId].name} requires ${WOODCUTTING.name} ${GEAR[gearId].requiredLevel}` : `Equip ${GEAR[gearId].name}`;
        return <button className={`inventory-slot inventory-slot--filled inventory-slot--gear inventory-slot--draggable ${locked ? "inventory-slot--locked" : ""} ${draggedSlot===index ? "inventory-slot--dragging" : ""}`} type="button" key={index}
          ...
          onClick={event => activateItem(itemId,index,event.timeStamp)} onContextMenu={event => onItemContext?.(event,itemId,index)} title={`${title}. Drag to rearrange.`} aria-label={`${title}. Drag to rearrange.`}>
          <GearIcon id={gearId}/><span className="inventory-equip-mark">{mode==="deposit" ? "↓" : mode==="sell" ? "¤" : locked ? GEAR[gearId].requiredLevel : "+"}</span>
        </button>;
      }
      if (itemId) {
        const title = mode==="deposit" ? `Deposit ${ITEMS[itemId].name}` : mode==="sell" ? `Select ${ITEMS[itemId].name} to sell` : ITEMS[itemId].name;
        return <button className={`inventory-slot inventory-slot--filled inventory-slot--draggable ${mode==="deposit" ? "inventory-slot--actionable" : ""} ${draggedSlot===index ? "inventory-slot--dragging" : ""}`} type="button" key={index}
          draggable onDragStart={event => {event.dataTransfer.effectAllowed="move";event.dataTransfer.setData("application/x-tallowmere-slot",String(index));setDraggedSlot(index);}}
          onDragEnd={event => {lastDragEndedAt.current=event.timeStamp;setDraggedSlot(null);}} onDragOver={event => event.preventDefault()} onDrop={event => dropItem(event,index)} onKeyDown={event => moveWithKeyboard(event,index)}
          onClick={event => mode!=="equip" && activateItem(itemId,index,event.timeStamp)} onContextMenu={event => onItemContext?.(event,itemId,index)} title={`${title}. Drag to rearrange.`} aria-label={`${title}. Drag to rearrange.`}>
